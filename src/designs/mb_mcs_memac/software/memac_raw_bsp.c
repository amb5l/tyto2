...
uint8_t memac_raw_get_speed(void) {
    return (gpi(1) >> MEMAC_GPIB_RX_SPD0) & 0b11;
}
...
uint8_t memac_raw_tx_rdy(void) {
    return gpi(1) & (1 << MEMAC_GPIB_TX_PRQ_RDY) ? 1 : 0;
}
...
void memac_raw_rx_ctrl(
    ...
    gpormw(1,
        ...
        (((ipg_min) & 0b1111) << MEMAC_GPOB_RX_IPG_MIN0) |
        (((pre_len) & 0b1111) << MEMAC_GPOB_RX_PRE_LEN0) |
        (((pre_inc) & 1) << MEMAC_GPOB_RX_PRE_INC) |
        (((fcs_inc) & 1) << MEMAC_GPOB_RX_FCS_INC)
    );
}

uint8_t memac_raw_rx_rdy(void) {
    return gpi(1) & (1 << MEMAC_GPIB_RX_PRQ_RDY) ? 1 : 0;
}

uint8_t memac_raw_rx_get(RxPktRsrvDesc_t *p) {
    p->flags = peek16(MEMAC_BASE_RX_PDQ + 4);

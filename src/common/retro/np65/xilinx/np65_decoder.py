################################################################################
## np65_decoder.py                                                            ##
## Generates np65_decoder.vhd from CSV                                        ##
## Xilinx (inferred async ROM) version                                        ##
################################################################################
...
f.write('-- Xilinx (inferred async ROM) version\n')
f.write('--------------------------------------------------------------------------------\n')
f.write('\n')
...
f.write('\n')
f.write('\ttype decode_rom_t is array(0 to 255) of std_logic_vector(%d downto 0);\n' % (bit_width-1))
f.write('\n')
f.write('\tsignal decode_rom : decode_rom_t := (\n')
for i in range(256):
    f.write('\t\t"'+instruction_vectors[i]+'"');
    if i < 255:
        f.write(',')
    f.write('\n')
f.write('\t);\n')
...
f.write('begin\n')
f.write('\n')
f.write('\tdata <= decode_rom(to_integer(unsigned(opcode)));\n')
f.write('\n')
n = 0
for i, bf_name in enumerate(bf_names):
	f.write('\t%s' % bf_name)
